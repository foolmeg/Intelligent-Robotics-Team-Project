...
GOAL_X = 4.0
GOAL_Y = 3.3   # z = 0.09 in the world, but we only use x, y here
...
# Walls corresponding one-to-one to those in the updated astar_map.wbt
# (cx, cy, sx, sy, yaw), where:
#   - (cx, cy) are the wall centre coordinates on the ground plane (x, y),
#   - sx is the wall thickness, sy is its length,
...
def _distance_point_to_wall_rect(x, y, wall):
    """
    Compute the shortest distance from a point (x, y) to a rectangular wall in
    the ground plane.
...
class AStarController:
    ...
    def run(self):
        """
        Main loop: run static A* once to plan a global path, then follow the path
        until the goal is reached, while performing local collision avoidance
        and recovery.
        """
